...
/***/ function(module, exports, __webpack_require__) {
	...
	var astar = __webpack_require__(291);
	var msgpack = __webpack_require__(292);
	var client;
	(function (client) {
	    var slice = [].slice;
	    var seed = 0;
	    ...
	    global.utils = {
	        set_by_path: function set_by_path(obj, path, value) {
	            if (path.indexOf('.')) {
	                var a = path.split('.');
	                var p = obj;
	        ...
	        ajax: function ajax(url, timeout) {
	            timeout = timeout || 60000;
	            return new Promise(function (resolve, reject) {
	                setTimeout(function () {
	                    reject(undefined);
	                }, timeout);
	                $.ajax({ url: url, success: function success(m) {
	                        resolve(m);
	                    } });
	            });
	        },
	        gen_gid: function gen_gid() {
	            seed = seed + 1;
	            return seed;
	        }
	    };
	    var uiroot = undefined;

	    var eidmap = {};
	    // timeout is msec
	    client.wait = function (eid, m, tick) {
	        if (eid === undefined) {
	            client.log('wait must has at least one params');return;
	        }
	        if (eidmap[eid] !== undefined) {
	            client.log('重复的等待事件');
	            return;
	        }
	        m = m || 1000; //1秒
	        var ts = client.wrap.msec;
	        var timeout = m + ts;
	        var tmp = { state: 0 };
	        eidmap[eid] = tmp;
	        //client.log('wait:', eidmap);
	        return new Promise(function (resolve, reject) {
	            var seq = client.wrap.update(function () {
	                if (tmp && tmp.state == 1) {
	                    var ret = tmp.ret;
	                    delete eidmap[eid];
	                    client.wrap.kick(seq);
	                    if (ret !== undefined) {
	                    ...
	                    if (client.wrap.msec >= timeout) {
	                        ...
	                        if (tick !== undefined && m > 0) {
	                            tick((client.wrap.msec - ts) / m);
	                        };
	                    }
	                }
	            });
	    ...
	    client.waitting = function (eid) {
	        return eidmap[eid] && true || false;
	    };
	    client.delay = function (timeout) {
	    ...
	    client.notify = function (eid) {
	        var tmp = eidmap[eid];
	        if (tmp !== undefined) {
	            client.log('notify: ', eid, tmp);
	            if (arguments.length > 1) {
	                tmp.ret = slice.call(arguments, 1);
	            }
	            tmp.state = 1;
	        }
	    };
	    client.work = function (func) {
	        setTimeout(func, 0);
	    };
	    client.parallel = function () {
	    ...
	    client.socket = function (host, port) {
	        return new Promise(function (resolve, reject) {
	            setTimeout(function () {
	                reject(undefined);
	            }, 30 * 1000);
	            client.websocket(host, port, function (socket) {
	                resolve(socket);
	            });
	        });
	    };
	                        ...
	                        while (1) {
	                            switch (_context.prev = _context.next) {
	                                case 0:
	                                    ...
	                                    data = srv.join(username);

	                                    standalone.join(username, {
	                                        call: function call(m) {
	                                            var a = slice.call(arguments, 1);
	                                            return new Promise(function (res, rej) {
	                                                //do something
	                                                var delay = Math.random() * 0.2 + 0.2;
	                                                var ts = client.wrap.msec;
	                                                setTimeout(function () {
	                                                    res(srv.call(username, ts, m, a));
	                                                }, delay * 1000);
	                                            });
	                                        },
	                                        async_call: function async_call(m) {
	                                            //异步调用
	                                            var a = slice.call(arguments, 1);
	                                            var cb = a.pop();
	                                            //do something
	                                        ...
	                                        notify: function notify(m) {
	                                            ...
	                                            var ts = client.wrap.msec;
	                                            setTimeout(function () {
	                                                srv.notify(username, ts, m, a);
	                                            }, delay * 1000);
	                                        }
	                                    }, data);
	                                    //返回一个session和一份数据
	                                    resolve(standalone.get(username));
	                                    _context.next = 13;
	                                    break;

	                                case 7:
	                                    _context.next = 9;
	                                    return client.socket(client.wrap.host, client.wrap.port);

	                                case 9:
	                                    socket = _context.sent;

	                                    client.log('new websockt', socket);
	                                    //登录,如果登录成功了,就返回一份数据
	                                    socket.call(username, passwd, function (data) {
	                                        client.log('login return', data);
	                                        if (data) {
	                                            var ctx = new Client.ClientContext(username, {
	                                                call: function call(m) {
	                                                    var a = slice.call(arguments, 1);
	                                                    var ts = client.wrap.msec;
	                                                    return new Promise(function (ss, jj) {
	                                                        socket.call.apply(socket, [ts, m, a, function (res) {
	                                                            ss(res);
	                                                        }]);
	                                                    });
	                                                },
	                                                ...
	                                                notify: function notify(m) {
	                                                    var a = slice.call(arguments, 1);
	                                                    var ts = client.wrap.msec;
	                                                    socket.notify(ts, m, a);
	                                                }
	                                            });
	                                            socket.on('recv', function () {
	                                                client.log('client recv msg');
	                                                ctx.notify.apply(ctx, arguments);
	                                            });
	                                            ctx.unpack(data);
	                                            var env = new Client.ClientEnv(ctx);
	                                            //tick
	                                            var tick_seq = client.wrap.update(function () {
	                                                env.tick(client.wrap.msec);
	                                            });
	                                            socket.on('close', function () {
	                                                client.wrap.kick(tick_seq);
	                                            });
	                                            resolve(ctx);
	                                        }
	                                    });
	    ...
	    client.wrap = {
	        msec: Date.now(),
	        adjust: function adjust(srv) {
	            dot.srv = srv;
	            dot.cli = Date.now();
	            client.wrap.msec = srv;
	        },
	        tick: function tick() {
	            client.wrap.msec = dot.srv + (Date.now() - dot.cli);
	            if (islocal) {
	                if (standalone !== undefined) {
	                    standalone.tick(client.wrap.msec);
	                }
	            }
	        ...
	        update: function update(func) {
	            var seq = utils.gen_gid();
	            tick_list.splice(tick_list.length, 0, [seq, func]);
	            return seq;
	        },
	        kick: function kick(seq) {
	            var len = tick_list.length;
	            for (var i = 0; i < tick_list.length; i++) {
	                if (tick_list[i][0] == seq) {
	                    tick_list.splice(i, 1);
	                    break;
	                }
	            }
	        },
	        register: function register(path, func) {
	            var f = function f() {
	                var args = slice.call(arguments, 0);
	                return new Promise(function (resolve, reject) {
	                    args.splice(args.length, 0, resolve, reject);
	                    func.apply(undefined, args);
	                });
	            };
	            utils.set_by_path(client, path, f);
	        },
	        map: function map(path, func) {
	            utils.set_by_path(client, path, func);
	        },
	        init: function () {
	            var ref = _asyncToGenerator(regeneratorRuntime.mark(function _callee2(cb) {
	                var loading, ret, ui;
	                return regeneratorRuntime.wrap(function _callee2$(_context2) {
	                    while (1) {
	                        switch (_context2.prev = _context2.next) {
	                            case 0:
	                                client.log('client init.....');
	                                uiroot = client.uiroot;
	                                uiroot.add = function (obj) {
	                                    return client.add(uiroot, obj);
	                                };
	                                uiroot.rm = function (obj) {
	                                    return client.rm(uiroot, obj);
	                                };
	                                uiroot.get = function (obj) {
	                                    return client.get(uiroot, obj);
	                                };

	                                loading = uiroot.add(new LoadingUI());


	                                client.log('client init.....111111111111');
	                                _context2.next = 9;
	                                return client.Theme.load('resource/default.thm.json');

	                            case 9:
	                                ret = _context2.sent;

	                                client.log('client init.....222222222222');
	                                client.log('加载主题完成', ret);
	                                loading.setProgress('加载主题完成');
	                                //var ret = await client.RES.loadConfig('resource/default.res.json', 'resource/');
	                                loading.setProgress('加载配置完成');
	                                //var ret = await client.RES.loadGroup('preload', function(c, t) {
	                                //client.log('加载资源', c, t);
	                                //loading.setProgress('加载图片资源(' + c + '/' + t + ')');
	                                //});
	                                uiroot.rm(loading);
	                                client.log(' 删除 loading....');
	                                //TODO debug info
	                                global.Debug = uiroot.add(new DebugUI());

	                                uiroot.ready();
	                                if (islocal) {
	                                    standalone = new Client.StandAloneEnv().prepare();
	                                    //模拟嘀嗒,100ms一个嘀嗒
	                                    setInterval(function () {
	                                        standalone.srvtick(client.wrap.msec);
	                                    }, 100);
	                                    //var ben = await server.login('ben');
	                                    client.log('login ben ok', ben);
	                                    ui = uiroot.add(new BattleUI(ben));

	                                    client.log('open ui for ben');
	                                    //var haha = await server.login('haha');
	                                    client.log('login haha ok', haha);
	                                } else {
	                                    //var hh = await server.login('xixihaha', '123');
	                                    //var hh = await server.login(Date.now().toString(), '123');
	                                    //client.log('login : ', hh);
	                                    //var ui = uiroot.add(new BattleUI(hh));
	                                }
	                                if (typeof cb === 'function') {
	                                    cb();
	                                }
	                                client.log('astar = ', astar);
	        ...
	        /* test parallel 
	        await client.parallel(async(resolve)=>{
	            await client.wait('111', 1000);
	            client.log('111 ok');
	            resolve();
	        }, async(resolve)=>{
	            await client.wait('222', 2000);
	            client.log('222 ok');
	            resolve();
	        }, (resolve)=>{
	            client.log('333 ok');
	         ...
	         client.log('parallel ok');
	        */
	        /*
	         * test websockt
	         islocal = false;
	         var hh = await server.login('xixihaha');
	        client.log('login : ', hh);
	         */
	        ...
	        decode: function decode(bin) {
	            return msgpack.decode(bin);
	        },
	        standalone: function standalone() {
	            islocal = true;
	        },
	        login: function () {
	            var ref = _asyncToGenerator(regeneratorRuntime.mark(function _callee4(username, passwd) {
	                var hh, ui;
	                return regeneratorRuntime.wrap(function _callee4$(_context4) {
	                    while (1) {
	                        switch (_context4.prev = _context4.next) {
	                            case 0:
	                                _context4.next = 2;
	                                return server.login(username, passwd);

	                            case 2:
	                                hh = _context4.sent;
	                                ui = uiroot.add(new BattleUI(hh));
...
/***/ function(module, exports, __webpack_require__) {
	...
	var bops = __webpack_require__(293);
...
/***/ function(module, exports, __webpack_require__) {
	...
	proto.from = __webpack_require__(294)
	proto.to = __webpack_require__(296)
	proto.is = __webpack_require__(298)
	proto.subarray = __webpack_require__(299)
	proto.join = __webpack_require__(300)
	proto.copy = __webpack_require__(301)
	proto.create = __webpack_require__(302)

	mix(__webpack_require__(303), proto)
	mix(__webpack_require__(305), proto)
...
/***/ function(module, exports, __webpack_require__) {
	...
	var base64 = __webpack_require__(295)
	  , toutf8 = __webpack_require__(297)
